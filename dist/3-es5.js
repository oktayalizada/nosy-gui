...
var EmailTemplate = /** @class */ (function () {
    function EmailTemplate(emailFromProvider, emailTemplateName, emailTemplateTo, fromAddress, priority, retryPeriod, retryTimes, subject, text) {
    ...
    EmailTemplate.prototype.setEmailCc = function (cc) {
        this.cc = cc;
    };
    EmailTemplate.ctorParameters = function () { return [
        { type: String },
        { type: String },
        { type: Array },
        { type: String },
...
var EmailComponent = /** @class */ (function () {
    function EmailComponent(nosyHttpService, emailHttpService) {
    ...
    EmailComponent.prototype.ngOnInit = function () {
        ...
        this.emailHttpService.getAllAvailableProviders().subscribe(function (data) {
            _this.availableProviders = data;
        });
    };
    EmailComponent.prototype.onClearInputSystem = function () {
        this.isSelectedEmailTemplate = false;
        this.isSelectedInputSystem = false;
    };
    EmailComponent.prototype.onClearEmailTemplate = function () {
        this.isSelectedEmailTemplate = false;
    };
    EmailComponent.prototype.onChangeInputSystemFirstForm = function () {
        var _this = this;
        if (this.selectedInputSystemId !== null) {
            ...
            this.emailTemplates = [];
            this.isSelectedEmailTemplate = false;
            this.emailHttpService.getEmailTemplates(this.selectedInputSystemId).subscribe(function (data) {
                _this.emailTemplates = data;
                if (_this.emailTemplates.length > 0) {
                    _this.isSelectedInputSystem = true;
                    _this.isSelectedEmailTemplate = false;
                }
            });
        }
    };
    EmailComponent.prototype.onChangeEmailTemplateFirstForm = function (selectedEmailTemplate) {
        this.allFormReset();
        this.selectedEmailTemplate = selectedEmailTemplate;
        this.isSelectedEmailTemplate = true;
    };
    EmailComponent.prototype.allFormReset = function () {
        this.secondForm('', '', false);
        this.firstForm('', '', false);
    };
    EmailComponent.prototype.firstForm = function (colour, message, isEnabled) {
    ...
    EmailComponent.prototype.secondForm = function (colour, message, isEnabled) {
        this.colourPostSuccessMessage = colour;
        this.isPostSuccess = isEnabled;
        this.postSuccessMessage = message;
    };
    EmailComponent.prototype.updateExistingTemplateFirstForm = function (form) {
        var _this = this;
        var formInputSystem = form.value;
        if (formInputSystem.emailTemplateName === '' || formInputSystem.fromAddress === '' ||
            ...
            var emailTemplate = void 0;
            var emailTo = formInputSystem.emailTemplateTo1.trim().split(',');
            ...
            if (formInputSystem.emailTemplateCc1 !== '') {
                var emailCc = formInputSystem.emailTemplateCc1.trim().split(',');
                emailCc.forEach(function (part, index, theArray) {
                    theArray[index] = theArray[index].trim();
                });
                emailTemplate.setEmailCc(emailCc);
            }
            this.emailHttpService.updateEmailTemplate(emailTemplate, this.selectedInputSystemId, this.selectedEmailTemplateId).subscribe(function (response) {
                _this.firstForm('green', 'Email Template '
                    + _this.selectedEmailTemplate.name + ' updated!', true);
                _this.selectedEmailTemplate = response;
                _this.emailTemplates.splice(_this.emailTemplates.indexOf(_this.emailTemplates.find(function (x) { return x.id === _this.selectedEmailTemplateId; })), 1);
                _this.emailTemplates = _this.emailTemplates.concat([response]);
            }, function (err) {
                _this.firstForm('red', err.error.message, true);
            });
        }
    };
    EmailComponent.prototype.deleteExistingTemplate = function () {
        var _this = this;
        this.emailHttpService.deleteEmailTemplate(this.selectedEmailTemplateId, this.selectedInputSystemId).subscribe(function (response) {
            _this.firstForm('green', 'Successfully Deleted ' + _this.selectedEmailTemplate.name + 'from the System', true);
            _this.emailTemplates.splice(_this.emailTemplates.indexOf(_this.emailTemplates.find(function (x) { return x.id === _this.selectedEmailTemplateId; })), 1);
            _this.emailTemplates = _this.emailTemplates.slice();
        });
    };
    EmailComponent.prototype.onAddNewEmailTemplate = function (form) {
        var _this = this;
        var newTemplate = form.value;
        ...
        if (newTemplate.inputSystemIdFotTemplateCreation === '' || newTemplate.newEmailTemplateName === '' ||
            ...
            var emailCc = newTemplate.newEmailCc.trim().split(',');
            var emailTo = newTemplate.newEmailTo.trim().split(',');
            var emailTemplate_1;
            emailTo.forEach(function (part, index, theArray) {
                theArray[index] = theArray[index].trim();
            });
            emailTemplate_1 = new _email_template__WEBPACK_IMPORTED_MODULE_4__["EmailTemplate"](newTemplate.newFromProvider, newTemplate.newEmailTemplateName, emailTo, newTemplate.newFromAddress, 0, 0, 0, newTemplate.newSubject, newTemplate.newEmailText);
            if (newTemplate.newEmailCc !== '') {
                var emailCc_1 = newTemplate.newEmailCc.trim().split(',');
                emailCc_1.forEach(function (part, index, theArray) {
                    theArray[index] = theArray[index].trim();
                });
                emailTemplate_1.setEmailCc(emailCc_1);
            }
            this.emailHttpService.postNewTemplate(emailTemplate_1, this.inputSystemIdFotTemplateCreationId).subscribe(function (response) {
                _this.secondForm('green', 'New Template: ' + emailTemplate_1.name + ' created', true);
            }, function (err) {
                _this.secondForm('red', err.error.message, true);
            });
        }
    };
    EmailComponent.prototype.newTemplateInputSystemOnChange = function (newTemplateInputSystemOnChange) {
